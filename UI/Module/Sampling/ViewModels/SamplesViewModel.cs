  ...
  internal sealed class SamplesViewModel : ViewModelBase, ISamplesViewModel
  {
      ...
      )
    {
      ...
      ShareParameters = ReactiveCommand.Create(
        ...
        this.WhenAny(
          vm => vm.Distributions,
          _ => _moduleState.FilteredSamplesState.IsEnabled &&
      ...
      _subscriptions = new CompositeDisposable(
        ...
        _moduleState.FilteredSamplesState
          .ObservableForProperty(fss => fss.IsEnabled)
          .Subscribe(
            _reactiveSafeInvoke.SuspendAndInvoke<object>(
              ObserveFilterSamplesStateIsEnabled
              )
            ),
    ...
    private void ObserveFilterSamplesStateIsEnabled(object _)
    {
    ...
    private void PopulateSamples(DataTable? samples)
    {
      ...
      if (!_moduleState.FilteredSamplesState.IsEnabled || _moduleState.OutputFilters.IsEmpty)
      {
        dataTable = samples;
      }
      else
